...
## Why do you care?   
This topic interests us because we believe that 
...
\hfill\break
Similarly,  the three drug types perform fairly, with some slight variations that are difficult to distinguish whether the differences can be attributed to randomness or not.
\hfill\break
...
drugs.polr.long %>%
  ...
  scale_y_continuous(breaks = seq(0,1,0.5), sec.axis = sec_axis(~.*1, "Side Effects")) +
...
This plot shows the probabilites from the ordinal model. We can see that the model generally predicts high satisfaction as effectiveness and side effects increase. The probabilites are all generally low though, which suggests that it's pretty hard to accurately make a prediction based on this data alone. 

### Multinomial Logistic Regression Model

We also created multinomial models to see how well it would do treating the response as unordered. We made a few models all containing side effects and effectiveness as predictors. We also exmained drug type, the drug itself, and interactions between drug type and effectiveness and side effects. Once again we went with the simplest model since it had the best AIC and not a significantly worse deviance.

```{r MULTINOMIAL make model} 
multi_mod <- VGAM::vglm(factor(rating) ~ as.factor(effectiveness) + as.factor(sideEffects), data = df, family = "multinomial")
#deviance(multi_mod)
```

```{r MULTINOMIAL make df}
...
The mulitnomial model appears to be a little more decisive than the ordinal models, with most of the facets having a clear peak probability for one of the ratings. The deviance of this model is 1218.239, which is still extremely high, so this model probably isn't great for prediction.

```{r cut single model EFF}
