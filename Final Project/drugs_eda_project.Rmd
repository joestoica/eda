...
Overall Satisfaction was rated on a scale of 1 to 10, with 10 representing highest possible satisfaction. Perceived Effectiveness and Side Effects ratings were given on a 5-step Likert response scale. The remaining review aspects (benefits comment, side effects comment, overall comments) were written responses and excluded from our analyses.
...
This mosaic plot shows that the distribution of overall satisfaction ratings do not systematically vary across the major drug categories. Though the "Other" category appears to have marginally better overall satisfaction ratings, this may be attributed to greater noise due to the relatively small sample size.

```{r MOSAIC for SE and EFF} 
# Mosaic plot of user ratings of drug effectiveness and side effects by drug type (SSRI vs SNRIs)
# keep
...
Likewise, there does not appear to be any systematic differences in the distribution of ratings for perceived effectiveness or side effects between drug categories. As stated previously, the marginal improvements in ratings for "Other" drug category may simply be attributed to noise. 
...
drugs.polr.long %>%
  ...
  labs(title = "Proportional Odds Logistic Model: Satisfaction ~ Side Effects + Effectiveness")+
...
This plot shows the proportional odds logistic model fits, where it is apparent that the model generally predicts high satisfaction with improved effectiveness and side effects ratings. For ratings of effectiveness and side effects that are extreme in consistent directions (e.g. effectiveness & side effects = 5), the model provides very robust predicted probabilities for  satsifaction scores. Conversely, when effectiveness and side effects ratings are intermediate in intensity, predicted satisfaction ratings are markedly diffuse across the rating scale. This indicates that there is much less agreement about satisfaction among patients who gave moderate ratings for perceived effectiveness and severity of side effects.
...
polr4.long %>%
  ...
  labs(title = "Proportional Odds Logistic Model: Perceived Effectiveness ~ Side Effects") +
  guides(color = guides(col=FALSE)) + 
  scale_colour_viridis_d() +
  ...
  theme(plot.subtitle = element_text(size = 8),
        ...
        axis.text = element_text(size = 12)) 
```
Here we show model fits showing the degree to which side effects predict perceived drug effectiveness. It is clear that there is a strong relationship between perceived effectiveness and drug side effects, where the worst effectiveness ratings have the highest predicted probabilities when side effects are rated as extremely severe. These results show support the notion that drug side effects are in an important factor in the patients' evaluation of the extent to which the drug is actually addressing their underlying needs (depression).
