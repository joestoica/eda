...
df <- drugs %>% 
  ...
  select(-sideEffectsReview, -commentsReview, -X1, -condition) %>% 
...
x$drug_type = factor(x$drug_type, levels = c("NRI", "SRI", "Other"))
```
...
temp$drug = factor(temp$drug, levels = temp[order(temp$freq), 'drug'])

temp %>% 
  ggplot(aes(drug, freq)) +
