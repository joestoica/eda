...
temp$drug = factor(temp$drug, levels = c("concerta", "desyrel", "serzone", "symbyax", 
"trazodone", "pristiq", "sarafem", "celexa", "citalopram", "cymbalta", 
"zoloft", "paxil", "prozac", "lexapro", "effexor", "wellbutrin"))
...
```{r}
df <- df %>% as.tibble()
drugs_polr <- MASS::polr(factor(rating) ~ as.numeric(effectiveness) + as.numeric(sideEffects), data = df)
summary(drugs_polr)
```
