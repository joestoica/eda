...
df <- drugs %>% 
  ...
  mutate(sideEffects = case_when(
    sideEffects == "Extremely Severe Side Effects" ~ 5, 
    sideEffects == "Severe Side Effects" ~ 4, 
    sideEffects == "Moderate Side Effects" ~ 3, 
    sideEffects == "Mild Side Effects" ~ 2, 
    TRUE ~ 1)) %>% 
...
library(plyr)
df %>% 
  select(type, sideEffects) %>%
  gather("key", "value", sideEffects)
...
```{r}
alligator <- import("https://jfukuyama.github.io/teaching/stat670/notes/alligator.txt")
library(ggmosaic)
ggplot(alligator) +
...
x <- df %>% 
  ...
  gather("rating_type", "given_score", sideEffects, effectiveness, rating) %>% 
